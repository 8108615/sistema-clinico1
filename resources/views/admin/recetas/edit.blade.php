<x-layouts::app title="Editar Receta Médica">
    <flux:breadcrumbs>
        ...
        <flux:breadcrumbs.item>Edición de Receta</flux:breadcrumbs.item>
    </flux:breadcrumbs>
    <br>
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="historia_clinica_id" id="historia_clinica_id" class="w-full mt-1 border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-800 dark:text-zinc-100 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500" required>
                            ...
                            @foreach($historias as $historia)
                                <option value="{{ $historia->id }}" {{ old('historia_clinica_id', $receta->historia_clinica_id) == $historia->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <flux:input type="date" name="fecha_receta" value="{{ old('fecha_receta', $receta->fecha_receta) }}" required />
                        <flux:error name="fecha_receta" />
                    </div>

                    <div class="md:col-span-2">
                        <flux:label>Indicaciones Generales</flux:label>
                        <textarea name="indicaciones_generales" rows="2" placeholder="Indicaciones generales para el paciente..." class="w-full mt-1 border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-800 dark:text-zinc-100 rounded-lg p-3 text-sm focus:ring-2 focus:ring-blue-500">{{ old('indicaciones_generales', $receta->indicaciones_generales) }}</textarea>
                ...
                <div>
                    ...
                    <div id="contenedor-medicamentos" class="space-y-3">
                        @foreach($receta->detalles as $index => $detalle)
                            <div class="medicamento-row grid grid-cols-1 md:grid-cols-12 gap-2 p-3 bg-gray-50 dark:bg-zinc-900/50 border border-gray-200 dark:border-zinc-700 rounded-lg items-center">
                                <div class="md:col-span-4">
                                    <input type="text" name="medicamentos[{{ $index }}][medicamento]" value="{{ old('medicamentos.'.$index.'.medicamento', $detalle->medicamento) }}" placeholder="Medicamento" class="w-full border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-800 dark:text-zinc-100 rounded-lg px-3 py-1.5 text-sm" required>
                                </div>
                                <div class="md:col-span-3">
                                    <input type="text" name="medicamentos[{{ $index }}][dosis]" value="{{ old('medicamentos.'.$index.'.dosis', $detalle->dosis) }}" placeholder="Dosis" class="w-full border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-800 dark:text-zinc-100 rounded-lg px-3 py-1.5 text-sm" required>
                                </div>
                                <div class="md:col-span-2">
                                    <input type="text" name="medicamentos[{{ $index }}][duracion]" value="{{ old('medicamentos.'.$index.'.duracion', $detalle->duracion) }}" placeholder="Duración" class="w-full border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-800 dark:text-zinc-100 rounded-lg px-3 py-1.5 text-sm" required>
                                </div>
                                <div class="md:col-span-2">
                                    <input type="text" name="medicamentos[{{ $index }}][indicaciones]" value="{{ old('medicamentos.'.$index.'.indicaciones', $detalle->indicaciones) }}" placeholder="Indicación opcional" class="w-full border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-800 dark:text-zinc-100 rounded-lg px-3 py-1.5 text-sm">
                                </div>
    ...
    {{-- Script JavaScript para manejar los ítems dinámicos --}}
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            let contador = {{ $receta->detalles->count() }};
