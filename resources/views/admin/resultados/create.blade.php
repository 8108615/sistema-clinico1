<x-layouts::app title="Cargar Resultados de Laboratorio">
    ...
    <form action="{{ route('admin.resultados_laboratorios.store') }}" method="POST">
        ...
        <div class="p-6">
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mb-4">Ingresa o edita los valores obtenidos para cada parámetro del examen clínico.</p>

            <!-- Campos dinámicos o estructurados para los resultados -->
            <div class="space-y-4" id="parametros-container">
                @if(isset($detalleActual) && $detalleActual->resultados->count() > 0)
                    {{-- Si ya existen resultados guardados, los recorremos para mostrarlos --}}
                    @foreach($detalleActual->resultados as $index => $res)
                        <div class="p-4 border border-gray-200 dark:border-neutral-700 rounded-lg bg-white dark:bg-neutral-800 shadow-sm relative space-y-4 {{ $index > 0 ? 'mt-4' : '' }}">
                            @if($index > 0)
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <flux:input name="parametros[{{ $index }}][parametro]" value="{{ $res->parametro }}" placeholder="Ej. Glucosa, Leucocitos..." required />
                                </div>
                                <div>
                                    <flux:label>Resultado <span class="text-red-500">(*)</span></flux:label>
                                    <flux:input name="parametros[{{ $index }}][resultado]" value="{{ $res->resultado }}" placeholder="Ej. 95.5" required />
                                </div>
                                <div>
                                    <flux:label>Unidad de Medida</flux:label>
                                    <flux:input name="parametros[{{ $index }}][unidad_medida]" value="{{ $res->unidad_medida }}" placeholder="Ej. mg/dL, g/L" />
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <flux:input name="parametros[{{ $index }}][valores_referencia]" value="{{ $res->valores_referencia }}" placeholder="Ej. 70 - 110 mg/dL" />
                                </div>
                                <div>
                                    <flux:label>Observaciones</flux:label>
                                    <flux:input name="parametros[{{ $index }}][observaciones]" value="{{ $res->observaciones }}" placeholder="Observaciones adicionales..." />
                                </div>
                            </div>
                        </div>
                    @endforeach
                @else
                    {{-- Si no hay resultados previos, mostramos una tarjeta vacía por defecto --}}
...
{{-- Script nativo directo sin depender de stacks --}}
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Inicializamos el contador basado en las filas que ya existan en pantalla
        let contador = document.querySelectorAll('#parametros-container > div').length;
        const btnAgregar = document.getElementById('agregar-parametro');
        const container = document.getElementById('parametros-container');

        if (!btnAgregar || !container) return;
        ...
        btnAgregar.addEventListener('click', function() {
            ...
            const inputClasses = 'w-full rounded-md border border-gray-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 py-2 text-sm text-gray-800 dark:text-gray-100 shadow-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none';
            ...
            nuevoItem.innerHTML = `
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Parámetro <span class="text-red-500">(*)</span></label>
                        <input type="text" name="parametros[${contador}][parametro]" placeholder="Ej. Colesterol..." required class="${inputClasses}" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Resultado <span class="text-red-500">(*)</span></label>
                        <input type="text" name="parametros[${contador}][resultado]" placeholder="Ej. 200" required class="${inputClasses}" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Unidad de Medida</label>
                        <input type="text" name="parametros[${contador}][unidad_medida]" placeholder="Ej. mg/dL" class="${inputClasses}" />
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Valores de Referencia</label>
                        <input type="text" name="parametros[${contador}][valores_referencia]" placeholder="Ej. < 200 mg/dL" class="${inputClasses}" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Observaciones</label>
                        <input type="text" name="parametros[${contador}][observaciones]" placeholder="Observaciones adicionales..." class="${inputClasses}" />
                    </div>
                </div>
            `;
        ...
        // Asegurar que los botones de eliminar de los elementos precargados funcionen también
        document.querySelectorAll('.eliminar-parametro').forEach(function(btn) {
